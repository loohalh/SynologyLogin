...
nasWebmanFileBackup="/usr/syno/synoman/webman/login-backup.gz"
nasWebmanFile="/usr/syno/synoman/webman/"
nasTmpFile="/usr/syno/synoman/webman/tmp"
...
expect -c "
set timeout 30
spawn -noecho scp -r $sourceFile $user@$host:$nasWebmanFile
expect {
...
expect -c "
set timeout 30
spawn -noecho scp -r $user@$host:$nasWebmanFileBackup $loginFileBackup
expect {
\"*yes/no\" { send \"yes\r\"; exp_continue }
\"*?assword:\" { send \"$passwd\r\" }
}
